...
class FilesGetter:
    ...
    def set_file_reader(self):
        if self.extension == "csv":
            ...
            raise Exception("We can't determin the file reader for such extension " + self.extension)
...
class Stimulus:
    def __init__(self, condition, pupil_diameter, array_size, d_per_logMAR, test_stimulus='Landolt_C', dioptres_def=0,
                 dioptres_ob_ast=0, dioptres_ver_ast=0, dioptres_ver_tre=0, dioptres_ver_com=0, dioptres_ob_tre=0,
                 dioptres_ob_com=0, dioptres_sph=0,
                 stimulus_intensity=1, background_intensity=0, colour_channel=1):
        ...
        self.mx = (self.array_size[1]) / 2.0  # np.median(np.arange(diameter)) # midpoint of array
        self.my = (self.array_size[0]) / 2.0
        self.X, self.Y = np.meshgrid(np.arange(-self.mx, self.mx),
                                     np.arange(-self.my, self.my))  # This could be pre-calculated and held globally
        ...
        self.final_coordinates = self.final_d_per_logMAR * np.arange(-self.m_final_array, self.m_final_array)

        self.padded_array_size = 768
        self.convolution_array_size = 768
        self.convolution_coordinates = self.final_d_per_logMAR * np.arange(-self.convolution_array_size / 2,
                                                                           self.convolution_array_size / 2)
        self.final_coordinates_padded = self.final_d_per_logMAR * np.arange(-self.padded_array_size / 2,
                                                                            self.padded_array_size / 2)
    ...
    def circle(self, diameter):
        ...
        circle = 1 * (self.radial < r ** 2)  # 1 for everything inside the circle, 0 for everything outside
        return circle
    ...
    def make_Landolt_C(self, orientation, diameter):
        ...
        if orientation == 0:
            gap = 1 * (self.Y > 0) * (self.X > -gap_width / 2) * (self.X < gap_width / 2)
        if orientation == 1:
            gap = 1 * (self.X > 0) * (self.Y > -gap_width / 2) * (self.Y < gap_width / 2)
        if orientation == 2:
            gap = 1 * (self.Y < 0) * (self.X > -gap_width / 2) * (self.X < gap_width / 2)
        if orientation == 3:
            gap = 1 * (self.X < 0) * (self.Y > -gap_width / 2) * (self.Y < gap_width / 2)
    ...
    def make_Single_point(self, diameter):
        ...
        stimulus[int(round((self.my) - logMAR_width / 2)):int(round((self.my) + logMAR_width / 2)),
        int(round((self.mx) - logMAR_width / 2)):int(round((self.mx) + logMAR_width / 2))] = 1
    ...
    def dioptres_to_RMS(self, dioptres):
        ...
        RMS = ((r ** 2) * dioptres) / (4 * math.sqrt(3))  # calculation of RMS in m (RMS = root mean square error)
    ...
    def calculate_zernike(self, n, m):
        ...
        for k in range(0, ((n - abs_m) // 2) + 1, 1):
            R += ((-1) ** k * math.factorial(n - k) * self.R ** (n - 2 * k)) / (
                    math.factorial(k) * math.factorial((n + abs_m) / 2 - k) * math.factorial((n - abs_m) / 2 - k))
        if m == 0:
            delta = 1.0
        else:
            delta = 0.0
    ...
    def mono_PSF(self, wavelength, dioptres_def, dioptres_ob_ast, dioptres_ver_ast, dioptres_ver_tre, dioptres_ver_com,
                 dioptres_ob_tre, dioptres_ob_com, dioptres_sph):
        print (wavelength)
        ...
        b_array_size = np.array(
            [int(np.rint(self.alpha * self.array_size[0])), int(np.rint(self.alpha * self.array_size[1]))])
        print("b_array_size: ", b_array_size)
        wavefront = self.apply_defocus(dioptres_def) + self.apply_ob_astigmatism(
            dioptres_ob_ast) + self.apply_ver_astigmatism(dioptres_ver_ast) + self.apply_ver_tref(
            dioptres_ver_tre) + self.apply_ver_coma(dioptres_ver_com) + self.apply_ob_tref(
            dioptres_ob_tre) + self.apply_ob_coma(dioptres_ob_com) + self.apply_sphe(dioptres_sph)
    ...
    def make_fft_PSF(self):
        ...
        if self.condition == 'rendered_mono':
            PSF, self.b_array_size, self.d_per_logMAR = self.mono_PSF(self.wavelengths_m[self.colour_channel],
                                                                      self.dioptres_def, self.dioptres_ob_ast,
                                                                      self.dioptres_ver_ast, self.dioptres_ver_tre,
                                                                      self.dioptres_ver_com, self.dioptres_ob_tre,
                                                                      self.dioptres_ob_com, self.dioptres_sph)
            PSF = self.scale_PSF(PSF, self.b_array_size, self.d_per_logMAR)
            PSF = PSF / np.sum(PSF)

            fft_psf = rfft2(PSF, [int(self.convolution_array_size), int(self.convolution_array_size)])
            # self.fft_psf = np.fft.fftshift(fft_psf)
            self.fft_psf = fft_psf

        elif self.condition == 'rendered_chromablur':
            focus = 1.7312 - ((633.46) / (
                    self.wavelengths_nm[1] - 214.10))  # equation and peak focus from marimont and wandell
            ...
            red_PSF, self.red_b_array_size, self.red_d_per_logMAR = self.mono_PSF(self.wavelengths_m[0],
                                                                                  self.combine_def_LCA(
                                                                                      self.dioptres_def, red_LCA),
                                                                                  ...
                                                                                  self.dioptres_sph)
            red_PSF = self.scale_PSF(red_PSF, self.red_b_array_size, self.red_d_per_logMAR)
            ...
            green_PSF, self.green_b_array_size, self.green_d_per_logMAR = self.mono_PSF(self.wavelengths_m[1],
                                                                                        self.combine_def_LCA(
                                                                                            self.dioptres_def,
                                                                                            green_LCA),
                                                                                        self.dioptres_ob_ast,
                                                                                        ...
                                                                                        self.dioptres_ob_com,
                                                                                        self.dioptres_sph)
            green_PSF = self.scale_PSF(green_PSF, self.green_b_array_size, self.green_d_per_logMAR)
            ...
            blue_PSF, self.blue_b_array_size, self.blue_d_per_logMAR = self.mono_PSF(self.wavelengths_m[2],
                                                                                     self.combine_def_LCA(
                                                                                         self.dioptres_def, blue_LCA),
                                                                                     self.dioptres_ob_ast,
                                                                                     self.dioptres_ver_ast,
                                                                                     self.dioptres_ver_tre,
                                                                                     self.dioptres_ver_com,
                                                                                     ...
                                                                                     self.dioptres_ob_com,
                                                                                     self.dioptres_sph)
            blue_PSF = self.scale_PSF(blue_PSF, self.blue_b_array_size, self.blue_d_per_logMAR)
    ...
    def scale_PSF(self, PSF, b_array_size, d_per_logMAR):
        coordinates_y = d_per_logMAR * (np.arange(-(b_array_size[0] - 1) / 2, ((b_array_size[0] - 1) / 2 + 1), 1))
        coordinates_x = d_per_logMAR * (np.arange(-(b_array_size[1] - 1) / 2, ((b_array_size[1] - 1) / 2 + 1), 1))
    ...
    def blur_image(self, wavelength, Landolt_C, fft_psf):
        ...
        stimulus = np.zeros((self.convolution_array_size, self.convolution_array_size)) + self.background_intensity
    ...
    def make_stimulus(self, orientation, diameter, Landolt_C="empty"):
        ...
        self.stimulus = np.zeros((self.final_array_size[0], self.final_array_size[1], 3))
        ...
        if self.condition == 'optical_mono':
            ...
            self.stimulus[:, :, self.colour_channel] = self.stimulus[:, :, self.colour_channel]
            ...
            if padding == 0:
                self.stimulus[:, :, self.colour_channel] = self.blur_image(self.wavelengths_m[self.colour_channel],
                                                                           Landolt_C, self.fft_psf)
            else:
                self.stimulus[padding:-padding, :, self.colour_channel] = self.blur_image(
                    self.wavelengths_m[self.colour_channel], Landolt_C, self.fft_psf)
            self.stimulus[:, :, self.colour_channel] = self.stimulus[:, :, self.colour_channel]
        elif self.condition == 'rendered_chromablur':
            # self.stimulus[:,:,:] = self.background_intensity
            if padding == 0:
                self.stimulus[:, :, 0] = self.blur_image(self.wavelengths_m[0], Landolt_C, self.red_fft_psf)
                self.stimulus[:, :, 1] = self.blur_image(self.wavelengths_m[1], Landolt_C, self.green_fft_psf)
                self.stimulus[:, :, 2] = self.blur_image(self.wavelengths_m[2], Landolt_C, self.blue_fft_psf)
            else:
                self.stimulus[padding:-padding, :, 0] = self.blur_image(self.wavelengths_m[0], Landolt_C,
                                                                        self.red_fft_psf)
                self.stimulus[padding:-padding, :, 1] = self.blur_image(self.wavelengths_m[1], Landolt_C,
                                                                        self.green_fft_psf)
                self.stimulus[padding:-padding, :, 2] = self.blur_image(self.wavelengths_m[2], Landolt_C,
                                                                        self.blue_fft_psf)
            self.stimulus[:, :, :] = self.stimulus[:, :, :]
...
def Write_Result(arc, user_res):
    ...
    f.write(str(LogM) + "\t" + "\t" + "\t" + str(deg) + "\t" + "\t" + "\t" + str(arc) + "\t" + "\t" + "\t" + str(
        dia) + "\t" + "\t" + "\t" + str(user_res) + "\n")
    f.close()
...
def QuestStairCase():
    ...
    minArcMin = 0.3
    maxArcMin = 8
    staircase = data.QuestHandler(guessGapArcmin, guessSD, pThreshold=0.63, gamma=0.25, method='mean', nTrials=15,
                                  minVal=minArcMin, maxVal=maxArcMin)
    for thisGapArcmin in staircase:
        ...
        while running:
            ...
            if e.type == pygame.KEYDOWN:
                if e.key == pygame.K_DOWN:  # if Landolt C orientation is up press arrow up
                    running = False
                    if orient == 3:  # up = 3
                        print ("correct")
                        #
                        FinalResult.append(get_result_for_one_iter(Arcmin, 1))
                        Write_Result(Arcmin, 1)  # if the result is correct print number 1
                        staircase.addResponse(1)
                    else:
                        print("incorrect")
                        FinalResult.append(get_result_for_one_iter(Arcmin, 0))
                        Write_Result(Arcmin, 0)  # if the result is incorrect print number 0
                        staircase.addResponse(0)
                elif e.key == pygame.K_UP:  # if Landolt C orientation is down press arrow down
                    running = False
                    if orient == 1:  # down = 1
                        print ("correct")
                        FinalResult.append(get_result_for_one_iter(Arcmin, 1))
                        ...
                        staircase.addResponse(0)
                elif e.key == pygame.K_LEFT:  # if Landolt C orientation is right press arrow right
                    running = False
                    if orient == 0:  # right = 0
                        print ("correct")
                        FinalResult.append(get_result_for_one_iter(Arcmin, 1))
                        ...
                        Write_Result(Arcmin, 0)
                        staircase.addResponse(0)
                elif e.key == pygame.K_RIGHT:  # if Landolt C orientation is left press arrow left
                    running = False
                    if orient == 2:  # left = 2
                        print ("correct")
                        FinalResult.append(get_result_for_one_iter(Arcmin, 1))
...
def experiment(filename, flag, result_folder):
    ...
    fail_iter_count = 0
    global C
    C = Stimulus(condition, pupil_diameter, array_size, d_per_Pixel, stimulus_intensity=0.8, background_intensity=0.2)
    C.make_fft_PSF()
    X1_time = time.time()
    #####################################
    ...
    global LC
    LC = Stimulus(condition, pupil_diameter_stimulus, array_size, d_per_Pixel, test_stimulus='Landolt_C',
                  dioptres_def=0, dioptres_ob_ast=0, dioptres_ver_ast=0,
                  dioptres_ver_tre=0, dioptres_ver_com=0, dioptres_ob_tre=0, dioptres_ob_com=0, dioptres_sph=0,
                  stimulus_intensity=1, background_intensity=0)
    ...
    wav_diop = 500 * 10 ** -9
    defocus_offset_stimulus = (((pupil_diameter_stimulus / 2) ** 2) * defocus_stimulus) / (4 * math.sqrt(3))
    defocus_offset_observer = (((pupil_diameter_observer / 2) ** 2) * defocus_observer) / (4 * math.sqrt(3))
    ...
    SStimulus = Stimulus(condition, pupil_diameter_stimulus, array_size, d_per_Pixel, test_stimulus='Single_point',
                         dioptres_def=XL_defocus * stimulus_hoa + defocus_offset_stimulus,
                         dioptres_ob_ast=XL_astig_O * stimulus_hoa, dioptres_ver_ast=XL_astig_V * stimulus_hoa,
                         dioptres_ver_tre=XL_tref_V * stimulus_hoa, dioptres_ver_com=XL_Coma_V * stimulus_hoa,
                         dioptres_ob_tre=XL_tref_O * stimulus_hoa, dioptres_ob_com=XL_coma_O * stimulus_hoa,
    ...
    global OObserver
    OObserver = Stimulus(condition, pupil_diameter_observer, array_size, d_per_Pixel, test_stimulus='Single_point',
                         dioptres_def=XL_defocus * observer_hoa + defocus_offset_observer,
                         dioptres_ob_ast=XL_astig_O * observer_hoa, dioptres_ver_ast=XL_astig_V * observer_hoa,
                         dioptres_ver_tre=XL_tref_V * observer_hoa, dioptres_ver_com=XL_Coma_V * observer_hoa,
                         dioptres_ob_tre=XL_tref_O * observer_hoa, dioptres_ob_com=XL_coma_O * observer_hoa,
    ...
    if flag == 4:
        combined_psf = signal.convolve2d(SStimulus_PSF_single, OObserver_PSF_single, boundary='symm', mode='same')
    ...
    global now
    now = datetime.now()
    FileName = "KhattarExp_" + now.strftime(
        "%H_%M_%S") + ".txt"  # KhatarExp file to save time and data on every experiment.
    f = open(result_folder + FileName, "w")  # type: # Results and Graphs folder has KhatarExp file.
    f.write(
        "logMAR" + "\t" + "\t" + "\t" + "\t" + "degree" + "\t" + "\t" + "\t" + "\t" + "\t" + "arcmin" + "\t" + "\t" + "\t" + "\t" + "\t" + "Pixels" + "\t" + "\t" + "\t" + "\t" + "Observation" + "\n")
    f.close()  # data units
...
def plot_figure(TrialList ,LogMArList, response_List, result_folder):
    ...
    if len(set(response_List)) == 1:
        ...
        if res == True:
            ...
            plt.plot(TrialList, LogMArList, marker='s', color="black", markerfacecolor='none', markeredgecolor='k')
    else:
...
if __name__ == "__main__":
    ...
    file = "C:\\Users\\admin-nreadlab\\Desktop\\eyes\\exp1\\eye_50.csv"
    #file = "C:/Users/admin-nreadlab/Desktop/eyes/Khatar_right_logfile_20_03_12_13_29_20 (1).csv"
    flag = 2

    result_folder = "C:\\Users\\admin-nreadlab\\Downloads\\Abi folder\\Results and Graphs"
